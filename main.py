...
def visualize_decision_boundary_embedded_space(
    ...
    gridpoints_embedded = polar_embedding(np.c_[gridpoints_x1.ravel(), gridpoints_x2.ravel()])
    new_feature_values = SVM.decision_function(gridpoints_embedded)
    new_feature_values = new_feature_values.reshape(gridpoints_x1.shape)
...
def visualize_H(X: np.ndarray, Y: np.ndarray) -> None:
    ...
    # Embed the data into the higher dimensional feature space
    embedded_data = polar_embedding(X)
    # Train the linear kernel svm ON THE EMBEDDED POINTS
    SVM_linear_kernel.fit(embedded_data, Y)
...
def visualize_slack_penalty(X: np.ndarray, Y: np.ndarray) -> None:
    ...
    # Define a list of C values to train an SVM with.
    ...
    for c in C_vals:
        ...
        predictions = svm_model.predict(gridpoints)
...
def visualize_support_vectors(X: np.ndarray, Y: np.ndarray) -> None:
    ...
    for i in range(N_REMOVALS):
        ...
        non_support_vectors = np.delete(X, SVM.support_, axis=0)
        non_support_vector_labels = np.delete(Y, SVM.support_)
