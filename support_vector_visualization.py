...
class SupportVectorVisualization(Visualizer):
    ...
    def generate_x2_vals(
        ...
        # Calculate the x2 values corresponding to the (x1, x2) points which lie on the decision boundary
        x2_vals = slope * x1_axis_points + (-intercept / coefficients[1])
        return x2_vals
    ...
    def generate_margin_points(
        ...
        """
        # Get the vertical distance from the hyperplane to the margin using equation (16) from the handout
        vertical_distance = 1 / np.linalg.norm(coefficients)
        # Calculate the margin points.
        upper_margin_vals = x2_vals + vertical_distance
        lower_margin_vals = x2_vals - vertical_distance
    ...
    def visualize_solution(
        ...
        support_vector_labels = Y[self.svm.support_]
        non_support_vector_labels = np.delete(Y, self.svm.support_)
